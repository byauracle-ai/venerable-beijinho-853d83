...
// ─── HERO — visible immediately, blurs/darkens as you scroll ─────────────────
//
// Key insight: The hero is a NORMAL section (no sticky/overflow tricks).
// The image stays fixed in place while the page scrolls over it.
// A scroll-driven overlay darkens + blurs the image as the user scrolls down.
// Text fades in as the image fades out. Zero blank space.
...
function Hero() {
  ...
  // Progress 0→1 as you scroll through the hero section
  const progress = Math.min(1, scrollY / vh)

  // Image: blurs and darkens from 50% scroll onwards
  const imgBlur = progress > 0.4 ? (progress - 0.4) / 0.6 * 18 : 0
  const imgDark = progress > 0.4 ? (progress - 0.4) / 0.6 * 0.92 : 0.15

  // Hero text: fades out in first 35% of scroll
  const textOpacity = Math.max(0, 1 - progress * 2.8)

  return (
    <section ref={ref} style={{ position: 'relative', height: '100dvh', overflow: 'hidden' }}>
      {/* Image — fills viewport exactly, objectFit cover preserves quality */}
      <div style={{ position: 'absolute', inset: 0 }}>
        <img
          ...
          style={{
            ...
            transform: `scale(${1 + imgBlur * 0.005})`, // compensate for blur edge
        ...
        <div style={{
          ...
          background: `rgba(4,4,10,${imgDark})`,
          transition: 'background 0.05s',
        }} />
        {/* Permanent bottom vignette for text legibility */}
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          color: 'var(--T)', margin: '0 0 18px', lineHeight: 1.5,
        ...
        <p style={{
          fontFamily: 'var(--F)', fontSize: 'clamp(14px,1.5vw,17px)', fontStyle: 'italic',
          color: 'rgba(242,237,230,0.6)', maxWidth: 380, lineHeight: 1.85, marginBottom: 36,
      ...
      {/* Yield badge */}
      <div style={{
        position: 'absolute', top: 88, right: 'clamp(24px,5vw,72px)',
        opacity: textOpacity,
        borderTop: '1px solid rgba(200,168,92,0.22)', borderBottom: '1px solid rgba(200,168,92,0.22)',
        padding: '16px 24px', textAlign: 'center',
        background: 'rgba(4,4,10,0.5)', backdropFilter: 'blur(12px)',
      ...
      <div style={{
        ...
        opacity: textOpacity * 0.6, pointerEvents: 'none',
...
// ─── SCENE — full viewport image that blurs/darkens on exit and reveals on entry
// Used for the post-hero cinematic panels. No sticky, no blank space.
// Each scene is exactly 100dvh. The blur/dark is driven by intersection.

function Scene({ src, label, caption, objPos = 'center', id }: {
  src: string; label?: string; caption?: string; objPos?: string; id?: string
}) {
  const ref = useRef<HTMLDivElement>(null)
  const [progress, setProgress] = useState(0) // 0 = fully visible, 1 = fully scrolled past
  ...
  useEffect(() => {
    const fn = () => {
      ...
      // -1 = above viewport, 0 = centred, 1 = below viewport
      const p = -rect.top / vh
  ...
  // As you scroll OUT of this scene (progress 0→1), it blurs + darkens
  const exitBlur = progress > 0.6 ? (progress - 0.6) / 0.4 * 16 : 0
  const exitDark = progress > 0.6 ? (progress - 0.6) / 0.4 * 0.9 : 0

  // Caption visible in lower half of viewport (progress 0.1→0.55)
  ...
  return (
    <section id={id} ref={ref} style={{ position: 'relative', height: '100dvh', overflow: 'hidden' }}>
      <div className="img-wrap" style={{ position: 'absolute', inset: 0 }}>
        <img
          ...
          style={{
            ...
            transform: `scale(${1 + exitBlur * 0.005})`,
            willChange: 'filter, transform',
          }}
        />
      ...
      {label && (
        <div style={{
          position: 'absolute', bottom: 'clamp(52px,7vh,88px)', left: 'clamp(40px,6vw,96px)',
          maxWidth: 560, opacity: capOp,
          transform: `translateY(${(1 - Math.min(1, capOp * 4)) * 10}px)`,
          pointerEvents: 'none',
        }}>
...
// ─── Full-bleed panel (for estate photos — not in descent sequence) ───────────
  ...
  return (
    <section id={id} style={{ position:'relative',height:'100dvh',overflow:'hidden',display:'flex',flexDirection:'column',justifyContent:'flex-end' }}>
      ...
      <div ref={ref} style={{
        ...
        padding: 'clamp(40px,6vw,96px)', paddingBottom: 'clamp(52px,7vw,96px)',
        ...a,
        ...
        {sub && <p style={{ fontFamily:'var(--F)',fontSize:'clamp(14px,1.5vw,18px)',fontStyle:'italic',color:'rgba(242,237,230,0.54)',marginTop:16,maxWidth:460,lineHeight:1.75,...(align==='center'?{margin:'16px auto 0',display:'block'}:{}) }}>{sub}</p>}
      </div>
    </section>
  )
}
...
function Island() {
  return (
    <section id="island">
      <Panel src={IMG.island} eyebrow="The Setting · Grand Baie" title="Minutes from one of the world's last untouched lagoons" sub="The northern lagoon — turquoise, warm, impossibly clear." objPos="center 30%" dim={0.5} />
...
function Wellness() {
  ...
  return (
    <section id="wellness" style={{ background:'var(--spa)' }}>
      <Dark center eyebrow="In-Residence Wellness" title="The Spa Sanctuary" body="Ancient Mauritian healing traditions, reborn in volcanic stone and total silence." />
      <Panel src={IMG.spa1}    eyebrow="Concierge"   title="Your Dedicated Curator"   sub="A personal wellness director — anticipating every need before it becomes one." objPos="center 30%" dim={0.52} />
      <Panel src={IMG.spa2}    eyebrow="Treatment"   title="The Treatment Sanctuary"  sub="Volcanic stone · Cold ocean mineral · Island botanicals." align="right" dim={0.52} />
...
function HomePage() {
  return (
    <>
      ...
      {/* HERO — staircase, visible immediately, blurs/darkens on scroll */}
      <Hero />

      {/* Post-hero scenes — each 100dvh, blur/dark on exit, no blank space */}
      ...
      <Scene src={IMG.seaView2} label="The Horizon"   caption="Unobstructed. Uncompromising. Yours."                 objPos="center 35%" />
      <Scene src={IMG.pool}     label="The Pool"      caption="Where the water ends and the ocean begins"            objPos="center 45%" id="estate" />

      {/* Estate */}
      <Specs />
      <Panel src={IMG.seaView}  eyebrow="The Estate"  title="Designed to disappear"        sub="Architecture that serves the view, not itself." />
      <Panel src={IMG.interior} eyebrow="Interior"    title="Every surface chosen"         sub="Reclaimed teak, Calacatta Oro, Belgian linen — nothing left to chance." align="right" dim={0.5} />
      <Panel src={IMG.pool2}    eyebrow="Blue Hour"   title="The pool at dusk"             sub="Salt air. Candlelight. The horizon on fire." align="center" dim={0.42} />
